...
export default function Result(props: {
  ...
  useEffect(() => {
    ...
    if (frm.price > 999) {
      if (sType) {
        setClosingFee(
          sType.PriceRange[sType.PriceRange.length - 1].fixedClosingFee
        );
      }
    } else if (sType) {
    ...
    <Table>
      ...
      <TableBody>
        ...
        <TableRow>
          ...
          <TableCell>
            ₹{(0.18 * (referralFee + closingFee + shippingFee)).toFixed(2)}
          </TableCell>
        </TableRow>
        <TableRow>
          <TableCell>Total Amazon Charges</TableCell>
          <TableCell>
            ₹{(1.18 * (referralFee + closingFee + shippingFee)).toFixed(2)}
          </TableCell>
        </TableRow>
      </TableBody>
      <TableFooter>
        <TableRow>
          ...
          <TableCell>
            {(
              frm.price -
              1.18 * (referralFee + closingFee + shippingFee)
            ).toFixed(2)}
          </TableCell>
        </TableRow>
      </TableFooter>
